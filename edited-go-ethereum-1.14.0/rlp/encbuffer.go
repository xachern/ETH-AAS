...
func (buf *encBuffer) writeBytes(b []byte) {
	if len(b) == 1 && b[0] <= 0x7F {
		//特殊情况：如果 b 只有一个字节，并且这个字节的值小于或等于 0x7F (即 127)，
		//	按照 RLP 规范，字节可以直接作为编码后的内容，不需要长度前缀。
		//	此时直接将字节 b[0] 添加到 buf.str 中
		// fits single byte, no string header
		buf.str = append(buf.str, b[0])
	} else {
		// 一般情况：调用 encodeStringHeader 来为该字符串编码添加长度前缀，然后将[]byte b 添加到buf
