...
// traverseNodeValue 递归遍历 Trie 所有叶子节点（valueNode）
// param path: keyHex , got by keybytesToHex(), len=65
// result: string(key) -> value, key is keyBytes, len=32
func (t *Trie) traverseNodeValue(current node, path []byte, collector map[string][]byte) error {
	switch n := current.(type) {
	case nil:
		return nil
	case valueNode:
		// 将 hex 路径转换为原始 key 格式（去除末尾的 16 终止符）
		rawKey := hexToKeybytes(path)
		val := n
		collector[string(rawKey)] = val
		return nil
	case *shortNode:
		// 拼接路径：shortNode 的 Key 是 hex 编码的半字节
		newPath := append(path, n.Key...)
		return t.traverseNodeValue(n.Val, newPath, collector)
	case *fullNode:
		// 遍历所有可能的子节点（16个半字节分支）
		for i := 0; i < 16; i++ {
			child := n.Children[i]
			if child != nil {
				...
				if err := t.traverseNodeValue(child, newPath, collector); err != nil {
					return err
				}
			}
		}
	...
	case hashNode:
		...
		return t.traverseNodeValue(resolved, path, collector)
...
// 返回所有storage slot的key-value
// result: string(key) -> value, key is keyBytes, len=32
func (t *Trie) MyGetAllLeaves() (map[string][]byte, error) {
	result := make(map[string][]byte)
	if err := t.traverseNodeValue(t.root, []byte{}, result); err != nil {
		return nil, err
	}
	return result, nil
}

// traverseNodeKey 递归遍历 Trie 所有叶子节点（valueNode）
// param path: keyHex , got by keybytesToHex(), len=65
// result: string(key), key is keyBytes, len=32
func (t *Trie) traverseNodeKey(current node, path []byte, collector []string) error {
	switch n := current.(type) {
	case nil:
		return nil
	case valueNode:
		// 将 hex 路径转换为原始 key 格式（去除末尾的 16 终止符）
		rawKey := hexToKeybytes(path)
		collector = append(collector, string(rawKey))
		return nil
	case *shortNode:
		// 拼接路径：shortNode 的 Key 是 hex 编码的半字节
		newPath := append(path, n.Key...)
		return t.traverseNodeKey(n.Val, newPath, collector)
	case *fullNode:
		// 遍历所有可能的子节点（16个半字节分支）
		for i := 0; i < 16; i++ {
			child := n.Children[i]
			if child != nil {
				// 追加当前半字节到路径
				newPath := append(path, byte(i))
				if err := t.traverseNodeKey(child, newPath, collector); err != nil {
					return err
				}
			}
		}
	...
	case hashNode:
		...
		return t.traverseNodeKey(resolved, path, collector)
	default:
		return fmt.Errorf("unknown node type: %T", n)
	}
}

// 返回所有storage slot的key
// result: string(key), key is keyBytes, len=32
func (t *Trie) MyGetAllKeys() ([]string, error) {
	result := make([]string, 0)
	if err := t.traverseNodeKey(t.root, []byte{}, result); err != nil {
		return nil, err
	}
	return result, nil
}

// result: array of depth of all keys
// func (t *Trie) traverseNodeDepth(current node, path []byte, collector []int) error {
// 	switch n := current.(type) {
// 	case nil:
// 		return nil
// 	case valueNode:
...
// func (t *Trie) MyGetAllDepth() ([]int, error) {
// 	result := make([]string, 0)
// 	if err := t.traverseNodeKey(t.root, []byte{}, result); err != nil {
// 		return nil, err
// 	}
