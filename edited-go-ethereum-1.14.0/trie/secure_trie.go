...
// read each storage slot in the storage trie
// result: string(key) -> value, key is keyBytes, len=32
func (t *StateTrie) MyTraverseStorageTrie() (map[string][]byte, error) {
	slots, err := t.trie.MyGetAllLeaves()
	if err != nil {
		return nil, err
	}
	return slots, nil
...
// read all existing storage key in the storage trie
// result: string(key), key is keyBytes, len=32
func (t *StateTrie) MyTraverseStorageTrieKeys() ([]string, error) {
	storage_keys, err := t.trie.MyGetAllKeys()
	if err != nil {
		return nil, err
	}
	return storage_keys, nil
}
