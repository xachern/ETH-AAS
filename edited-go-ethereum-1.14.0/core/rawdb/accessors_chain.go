...
// read both ancient and key-value store
func ReadHistoricalBlockInRange(db ethdb.Reader, startBlockNum uint64, count uint64) []*types.Block {
	var blocks []*types.Block
	if count == 0 {
		return nil
	}
	blknum := startBlockNum
	if count-1 > startBlockNum {
		// It's ok to request block 0, 1 item
		count = startBlockNum + 1
	}
	limit, _ := db.Ancients()
	// First read live blocks
	if blknum >= limit {
		// If we need to read live blocks, we need to figure out the hash first
		hash := ReadCanonicalHash(db, startBlockNum)
		for ; blknum >= limit && count > 0; blknum-- {
			// read header
			header_rlp_data := ReadHeaderRLP(db, hash, blknum)
			if len(header_rlp_data) == 0 {
				break // Maybe got moved to ancients
			}
			header := new(types.Header)
			if err := rlp.DecodeBytes(header_rlp_data, header); err != nil {
				log.Error("Invalid block header RLP", "hash", hash, "err", err)
				return nil
			}

			// read body
			body_rlp_data := ReadBodyRLP(db, hash, blknum)
			if len(body_rlp_data) == 0 {
				break // Maybe got moved to ancients
			}
			body := new(types.Body)
			if err := rlp.DecodeBytes(body_rlp_data, body); err != nil {
				log.Error("Invalid block body RLP", "hash", hash, "err", err)
				return nil
			}

			block := types.NewBlockWithHeader(header).WithBody(body.Transactions, body.Uncles).WithWithdrawals(body.Withdrawals)
			blocks = append(blocks, block)

			// Get the parent hash for next query
			hash = types.HeaderParentHashFromRLP(header_rlp_data)
			count--
		}
	}
	if count == 0 {
	...
	header_rlp_data, err := db.AncientRange(ChainFreezerHeaderTable, blknum+1-count, count, 2*1024*1024)
	if err != nil {
		log.Error("Failed to read headers from freezer", "err", err)
		return blocks
	}
	if uint64(len(header_rlp_data)) != count {
		log.Warn("Failed to read enough headers from freezer (Limit 2M)", "wanted", count, "read", len(header_rlp_data))
		return blocks
	}
	body_rlp_data, err := db.AncientRange(ChainFreezerBodiesTable, blknum+1-count, count, 2*1024*1024)
	if err != nil {
		log.Error("Failed to read bodies from freezer", "err", err)
		return blocks
	}
	if uint64(len(body_rlp_data)) != count {
		log.Warn("Failed to read enough bodies from freezer (Limit 2M)", "wanted", count, "read", len(body_rlp_data))
		return blocks
	}
	// The data is on the order [h, h+1, .., n] -- reordering needed
	for i := range header_rlp_data {
		header := new(types.Header)
		if err := rlp.DecodeBytes(header_rlp_data[i], header); err != nil {
			log.Error("Invalid block header RLP", "err", err)
			return nil
		}
		body := new(types.Body)
		if err := rlp.DecodeBytes(body_rlp_data[i], body); err != nil {
...
// == cannot read ancient store, so avoid use this API for old blocks
// == only iterate key-value store, no support of ancient store
