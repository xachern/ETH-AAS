...
// /usr/local/go/bin/go test -timeout 60s -run ^Test_fetch_first_block$ github.com/ethereum/go-ethereum/core/rawdb
func Test_fetch_first_block(t *testing.T) {
	...
	chainDataPath := "/media/cxa/4T_ETH/eth/eth_test_hash_leveldb/geth/chaindata"
	db, err := NewLevelDBDatabase(chainDataPath, 1024, 300, "", true)
	if err != nil {
	...
	current_block_header := ReadHeadHeader(db)
	fmt.Printf("current_block_header: %v\n", current_block_header.Number)

	// fetch first block
	first_block_num := 1

	start := headerHashKey(uint64(first_block_num))
	...
	for it.Next() {
		...
		if key := it.Key(); len(key) == len(headerPrefix)+8+1 && bytes.Equal(key[len(key)-1:], headerHashSuffix) {
			blocknum := binary.BigEndian.Uint64(key[len(headerPrefix) : len(headerPrefix)+8])
