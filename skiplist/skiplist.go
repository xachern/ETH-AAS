...
func (list *SkipList) findPrev(start *Element, score float64, key interface{}) (elem *Element) {
	if list.length == 0 {
		return nil
	}
	...
	// Traverse each level to find the previous element
	for i >= 0 {
		for next := prevHeader.levels[i]; next != nil; next = prevHeader.levels[i] {
			if comp := list.compare(score, key, next); comp < 0 {
				break // We've gone too far, move to the next lower level
			} else {
				elem = next // Update the candidate for <= key
				prevHeader = &next.elementHeader
			}
		}

		// Skip levels that point to the same element
		topLevel := prevHeader.levels[i]
		for i--; i >= 0 && prevHeader.levels[i] == topLevel; i-- {
		}
	}
...
// FindPrev returns the last element before start that is less than or equal to key.
// If start is less than or equal to key, returns start.
// If there is no such element, returns nil.
// If start is nil, find element from back.
//
// The complexity is O(log(N)).
func (list *SkipList) FindPrev(start *Element, key interface{}) (elem *Element) {
	return list.findPrev(start, list.calcScore(key), key)
}
